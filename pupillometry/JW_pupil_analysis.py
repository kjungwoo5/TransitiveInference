...
# Defines variables based on modified parameters
if STAGE == 2:
    STIMULI = ['X', 'Normal', 'Deviant']
elif STAGE == 3:
...
def plot_by_session(aligned_pupil_by_session: dict):
    for session, value in aligned_pupil_by_session.items():
        pupil_plot = plt.subplots()
        print('plotting for session: ', session)
        ...
        for event_id, response in total_responses.items():
            ...
            pupil_plot[1].plot(baselined.columns, baselined.mean(axis=0), label=event_id)
            plot_shaded_error_ts(pupil_plot[1], baselined.columns, baselined.mean(axis=0),
                                 baselined.sem(axis=0), alpha=0.1)
        pupil_plot[1].legend()
        pupil_plot[1].set_xlim((PLOTTING_WINDOW[0], PLOTTING_WINDOW[1]))
        #pupil_plot[1].set_ylim((-0.5,0.9))
        annotation = f'n = {aligned_pupil_by_session[session]["X"].shape[0]} trials'
...
if PLOT_BY_SESSION:
    plot_by_session(aligned_pupil_by_session)
...
for event_id, response in aggregated_aligned_pupil.items():
    ...
    baselined = response.sub(baseline_mean, axis=0)
    pupil_plot[1].plot(baselined.columns, baselined.mean(axis=0),label=event_id)
    plot_shaded_error_ts(pupil_plot[1],baselined.columns,baselined.mean(axis=0), baselined.sem(axis=0),alpha=0.1)
pupil_plot[1].legend()
pupil_plot[1].set_xlim((PLOTTING_WINDOW[0], PLOTTING_WINDOW[1]))
...
fig = plt.gcf()
pupil_plot[0].show()
fig.savefig(fr'C:\Users\kjung\Documents\UCL\Year 4\ANAT0021 Dissertation\Coding\Analysis\Outputs\Stage 2 Baseline Regression\{animals_to_list}_Baseline_Subtracted.png')
...
fig.savefig(fr'C:\Users\kjung\Documents\UCL\Year 4\ANAT0021 Dissertation\Coding\Analysis\Outputs\Stage 2 Baseline Regression\{animals_to_list}_Coeff_Contribution.png')
...
fig = plt.gcf()
base_r2_plot[0].show()
fig.savefig(fr'C:\Users\kjung\Documents\UCL\Year 4\ANAT0021 Dissertation\Coding\Analysis\Outputs\Stage 2 Baseline Regression\{animals_to_list}_Coeff_R2.png')
...
fig = plt.gcf()
pupil_plot[0].show()
fig.savefig(fr'C:\Users\kjung\Documents\UCL\Year 4\ANAT0021 Dissertation\Coding\Analysis\Outputs\Stage 2 Baseline Regression\{animals_to_list}_Baseline_Regressed.png')
