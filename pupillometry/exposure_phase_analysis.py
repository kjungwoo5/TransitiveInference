...
def plot_by_session(aligned_pupil_by_session: dict):
    for session, value in aligned_pupil_by_session.items():
        ...
        pupil_plot[0].suptitle(f'Baseline subtracted plot of exposure phase for: {session}')
        pupil_plot[0].show()
...
for event_id, response in aggregated_aligned_pupil.items():
    ...
    plot_shaded_error_ts(pupil_plot[1],response.columns,response.mean(axis=0), response.sem(axis=0),alpha=0.1)
pupil_plot[1].legend()
pupil_plot[1].set_xlim((PLOTTING_WINDOW[0], PLOTTING_WINDOW[1]))
#pupil_plot[1].set_ylim((-0.6,0.2))
#pupil_plot[1].set_ylim((-0.25,0.75))
animals_to_list = ', '.join(ANIMALS)
pupil_plot[0].suptitle(f'Raw plot for {animals_to_list}')
pupil_plot[0].show()
...
#pupil_plot[1].set_ylim((-0.25,0.75))
animals_to_list = ', '.join(ANIMALS)
pupil_plot[0].suptitle(f'Baseline subtracted plot of exposure phase for {animals_to_list}')
pupil_plot[0].show()
...
base_eff_plot[0].suptitle(f'Baselined effects plot of exposure phase for {animals_to_list}')
...
base_r2_plot[0].suptitle(f'Baselined R2 plot of exposure phase for {animals_to_list}')
base_r2_plot[0].show()
...
pupil_plot[0].suptitle(f'Baseline Regressed plot of training stimuli of exposure phase for {animals_to_list}')
pupil_plot[0].show()
plt.pause(0.1)
...
#pupil_plot[1].set_ylim(Y_LIMS.get(animals_to_list, (-0.5,0.5)))
#pupil_plot[0].suptitle(f'Baseline Regressed plot for {animals_to_list}')
pupil_plot[0].suptitle(f'Baseline Regressed plot of exposure phase for {animals_to_list}')
pupil_plot[0].show()
plt.pause(0.1)'''
